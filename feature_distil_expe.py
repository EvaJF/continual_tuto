...
# -*- coding: utf-8 -*-
#
# Incremental fine-tuning with Knowledge Distillation on Features using MNIST dataset
# TODO : change implem of forward in CNN i order to separately output logits and features
...
from utils_tuto.encoder import compute_num_params, myCNN_features
...
from utils_tuto.loss import L2_loss, cos_loss, BalancedCrossEntropy
import pandas as pd
...
cumul = False 
CE_type = "balanced" # "classic" # "weighted", 
assert CE_type in ["weighted", "classic", "balanced"]
KD_type = "cos_features" # KL_logits, cos_logits / Also : features (cosine)
...
for step in range(nb_steps+1) : 
    ...
    if step == 0:
        ...
        # initialize model
        model = myCNN_features(nb_init_cl, size_conv_1, size_conv_2, size_fc)
    ...
    train_loader, val_loader, test_loader = get_MNIST_loaders_with_memory(train_range=train_cl, test_range=test_cl, memory=memory)
    ...
    if step > 0 :
        mu_CE = 0.90 #nb_incr_cl/nb_curr_cl #0.9 #  # TODO try other strategies
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (x, target) in enumerate(train_loader):
            ...
            out_dic = model(x) 
            out = out_dic["out"] # student logits
            feat = out_dic["features"] # features before classif   
            CE_loss = loss_fn(out, target)
            if step > 0 : 
                with torch.no_grad() :
                    ref_feat = ref_model(x)["features"] # teacher features
                KD_loss = cos_loss(feat, ref_feat)
